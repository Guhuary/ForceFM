...
from ..utils2.geometry import diff_between_two_axis_angle_numti
...
def _wrap_angle_diff(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
	"""Smallest difference on circle in [-pi, pi)."""
	return torch.atan2(torch.sin(a - b), torch.cos(a - b))


class GuidedNoiseTransform(NoiseTransform):
	"""CFM-style noise to x_t and builds a direct vector-field target.

	If `{name}_aug_transforms.pt` and `{name}_gnina_affinities.pt` exist under `pdbbind_dir/{name}/`,
	the target is a Boltzmann-weighted average over candidate x1 fields. Otherwise the target is the
	original crystal-pose CFM vector field.
	"""
	...
	def __init__(
		...
		guided_K: int,
		guided_k: float = 1.0,
		guided_weight_eps: float = 1e-8,
		guided_min_remaining: float = 1e-3,
	):
		super().__init__(t_to_sigma, no_torsion, all_atom)
		self.pdbbind_dir = pdbbind_dir
		self.guided_K = int(guided_K)
		self.guided_k = float(guided_k)
		self.guided_weight_eps = float(guided_weight_eps)
		self.guided_min_remaining = float(guided_min_remaining)

	def _set_gt_target(self, data):
		data.guided_train = False
		data.guided_target_tr = data.u_tr.clone()
		data.guided_target_rot = data.u_rot.clone()
		data.guided_target_tor = None if self.no_torsion or data.u_tor is None else data.u_tor.clone()
		return data

	def __call__(self, data):
		data = super().__call__(data)
		name = data['name']
		aug_path = os.path.join(self.pdbbind_dir, name, f'{name}_aug_transforms.pt')
		aff_path = os.path.join(self.pdbbind_dir, name, f'{name}_gnina_affinities.pt')

		if not (os.path.isfile(aug_path) and os.path.isfile(aff_path)):
			return self._set_gt_target(data)

		aug = torch.load(aug_path, weights_only=False)
		aff_blob = torch.load(aff_path, weights_only=False)
		aff = aff_blob['affinities']
		if not torch.is_tensor(aff):
			aff = torch.tensor(aff, dtype=torch.float32)
		else:
			aff = aff.float()

		tr_u = aug['tr_updates']
		rot_u = aug['rot_updates']
		tor_u = aug.get('tor_updates')
		if not torch.is_tensor(tr_u):
			tr_u = torch.tensor(tr_u, dtype=torch.float32)
		if not torch.is_tensor(rot_u):
			rot_u = torch.tensor(rot_u, dtype=torch.float32)
		if tor_u is not None and not torch.is_tensor(tor_u):
			tor_u = torch.tensor(tor_u, dtype=torch.float32)

		if rot_u.dim() == 3:
			rot_u = rot_u.squeeze(1)

		N = tr_u.shape[0]
		if N == 0 or rot_u.shape[0] != N or aff.shape[0] != N:
			log.warning(
				f'Invalid aug/affinity for {name} (N={N}); falling back to GT CFM targets.'
			)
			return self._set_gt_target(data)
		if tor_u is not None and tor_u.shape[0] != N:
			log.warning(
				f'Torsion aug count mismatch for {name} ({tor_u.shape[0]} vs {N}); falling back to GT CFM targets.'
			)
			return self._set_gt_target(data)

		k = min(self.guided_K, N)
		idx = torch.arange(N, dtype=torch.long) if k == N else torch.randperm(N)[:k]
		anchor_tr = tr_u[idx].float()
		anchor_rot = rot_u[idx].float()
		energy = aff[idx].float()

		remaining = max(float(1.0 - data.complex_t['tr'][0].item()), self.guided_min_remaining)
		current_tr = (-data.u_tr).squeeze(0).float() * remaining
		current_rot = (-data.u_rot).squeeze(0).float() * remaining

		u_tr = (anchor_tr - current_tr.unsqueeze(0)) / remaining
		rot_delta = diff_between_two_axis_angle_numti(
			current_rot.unsqueeze(0).expand(k, -1),
			anchor_rot,
		)
		u_rot = rot_delta / remaining

		log_q_tr = -((-u_tr) ** 2).sum(-1) / (2.0 * self.tr_sigma_max ** 2 + self.guided_weight_eps)
		log_q_rot = -((-u_rot) ** 2).sum(-1) / (2.0 * self.rot_sigma_max ** 2 + self.guided_weight_eps)
		log_q_tor = torch.zeros(k, dtype=torch.float32)

		if self.no_torsion or tor_u is None or data.u_tor is None:
			u_tor = None
		else:
			anchor_tor = tor_u[idx].float()
			current_tor = (-data.u_tor).float() * remaining
			u_tor = _wrap_angle_diff(anchor_tor, current_tor.unsqueeze(0)) / remaining
			log_q_tor = -((-u_tor) ** 2).sum(-1) / (2.0 * self.tor_sigma_max ** 2 + self.guided_weight_eps)

		weights = torch.softmax(log_q_tr + log_q_rot + log_q_tor - self.guided_k * energy, dim=0)

		data.guided_train = True
		data.guided_K = k
		data.guided_target_tr = (weights[:, None] * u_tr).sum(0, keepdim=True)
		data.guided_target_rot = (weights[:, None] * u_rot).sum(0, keepdim=True)
		data.guided_target_tor = None if u_tor is None else (weights[:, None] * u_tor).sum(0)
...
class PDBBind(Dataset):
	def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
				 ...
				 protein_path_list=None, ligand_descriptions=None, keep_local_structures=False, guided=False):
		...
		if self.guided:
			n_aug = 0
			for graph_path in self.complexes_names:
				bn = os.path.basename(graph_path)
				name = bn[len('heterograph_'):-len('.pt')]
				aug = os.path.join(self.pdbbind_dir, name, f'{name}_aug_transforms.pt')
				aff = os.path.join(self.pdbbind_dir, name, f'{name}_gnina_affinities.pt')
				if os.path.isfile(aug) and os.path.isfile(aff):
					n_aug += 1
			log.info(
				f'Guided PDBBind: {len(self.complexes_names)} complexes '
				f'({n_aug} with aug+gnina, {len(self.complexes_names) - n_aug} GT-only fallback)'
			)
...
def construct_dataset(args, t_to_sigma):
	...
	if guided:
		transform = GuidedNoiseTransform(
			no_torsion=args.no_torsion,
			all_atom=args.all_atoms,
				t_to_sigma=t_to_sigma,
				pdbbind_dir=args.data_dir,
				guided_K=getattr(args, 'guided_K', 8),
				guided_k=getattr(args, 'guided_k', 1.0),
				guided_weight_eps=getattr(args, 'guided_weight_eps', 1e-8),
				guided_min_remaining=getattr(args, 'guided_min_remaining', 1e-3),
			)
	else:
	...
	common_args = {'transform': transform, 'root': args.data_dir, 'limit_complexes': args.limit_complexes,
				   ...
				   'esm_embeddings_path': args.esm_embeddings_path, 'guided': guided}
	...
	test_dataset = PDBBind(cache_path=args.cache_path, split_path=args.split_test, keep_original=True, **common_args)
...
class PDBBindDataModule(LightningDataModule):
	def __init__(self, args):
		...
		self.train_dataset, self.valid_dataset, self.test_dataset = construct_dataset(args, self.t_to_sigma)
	...
	def test_dataloader(self):
		test_loader = DataLoader(self.test_dataset, batch_size=self.args.batch_size_per_device, num_workers=self.args.num_workers, shuffle=False)
		return test_loader
