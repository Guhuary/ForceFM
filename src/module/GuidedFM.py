...
class Guide_FM_Model(LightningModule):
    def __init__(self, args, basefm):
        super().__init__()
        self.args = args
        self.model = get_vector_field(args)

        self.basefm = basefm
        for param in self.basefm.parameters():
    ...
    def training_step(self, data, batch_idx):
        ...
        with torch.no_grad():
            tr_base, rot_base, tor_base = self.basefm(data)
        tr_pred = tr_pred / 10 + tr_base
        rot_pred = rot_pred / 10 + rot_base
        tor_pred = tor_pred / 10 + tor_base
        ...
        # self.print('train loss', loss.item(), 'tr', tr_loss.item(), 'rot', rot_loss.item(), 'tor', tor_loss.item())
